...
def train(
...
) -> tuple[MT5ForConditionalGeneration, T5TokenizerFast]:
    device = torch.device("cuda" if use_gpu else "cpu")
    logger.info("loading base T5 model")
    model = MT5ForConditionalGeneration.from_pretrained(base_model).to(device)
    ...
    trainer = pl.Trainer(
        ...
        precision=precision,
        log_every_n_steps=200,  ### replace log_every_n_steps=
